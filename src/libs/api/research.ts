...
export class ResearchAPI {
	...
	async getResearch(): Promise<IResearch[]> {
		...
		Object.keys(params).forEach((key) => this.researchEndpoint.searchParams.append(key, params[key]));
...
export class ResearchAPIV2 {
	...
	async getResearch(is_mini?: boolean, ids?: number[]): Promise<IResearchV2[]> {
		...
		if (is_mini) {
			const research: IResearchMini[] = data.sort((a: IResearchMini, b: IResearchMini) =>
