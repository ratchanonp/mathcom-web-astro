...
export class FacultyAPI {
	...
	async getFaculties(): Promise<IFaculty[] | undefined> {
		...
		Object.keys(params).forEach((key) => this.facultyEndpoint.searchParams.append(key, params[key]));
	...
	async getAllFacultySlugs(): Promise<string[]> {
		...
		Object.keys(params).forEach((key) => fetURL.searchParams.append(key, params[key]));
...
export class FacultyAPIV2 {
	...
	async getFaculties(ids?: number[]): Promise<IFacultyV2[] | undefined> {
		...
		try {
			...
			const faculties: IFacultyV2[] = data.sort((a: IFacultyV2, b: IFacultyV2) => a.title.localeCompare(b.title));
