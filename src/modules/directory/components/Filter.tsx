...
const Filter = () => {
	...
	const handleOnChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		if (e.target.checked) {
			...
			researchFieldSelected.set($filter.filter((id) => id !== parseInt(e.target.value)));
		}
	};
	...
	return (
		<div className="no-scrollbar mx-auto mt-5 max-h-96 max-w-screen-xl overflow-auto px-5 pb-5">
			...
			<form id="filter" className="mt-4 grid gap-5 md:grid-cols-2">
				{research.map(({ id, title }) => (
					<div key={id} className="flex flex-row-reverse items-center gap-2 md:flex-row">
						<input
							type="checkbox"
							id={id.toString()}
							value={id}
						...
						/>
						<label htmlFor={id.toString()} className="flex-1 font-light text-gray-400">
							{encodeStr.parseFromString(title, "text/html").body.textContent}
						</label>
					</div>
				))}
			</form>
