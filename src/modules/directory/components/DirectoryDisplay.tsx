    ...
    return faculties
      .filter((faculty) => {
        if (tab === "all" || tab.toLocaleLowerCase() === faculty.staff_type) {
          if (
            searchKeyword !== "" &&
            !faculty.title.toLowerCase().includes(searchKeyword.toLowerCase())
          ) {
            return false;
          }
      ...
      {tabsList.map((tab) => (
        <TabsContent key={tab.key} value={tab.value}>
          <div className="grid grid-cols-2 gap-5 p-5 md:grid-cols-3 lg:grid-cols-4">
            {filterFaculties(tab.key, $serchKeyword, $researchFields, $sortBy)
              .length === 0 ? (
              <p className="text-center text-gray-500 col-span-full h-[400px]">
                No results found
              </p>
            ) : (
              filterFaculties(tab.key, $serchKeyword, $researchFields, $sortBy).map(
                (faculty) => <FacultyCard key={faculty.id} faculty={faculty} />
              )
            )}
            {loading && <p>Loading...</p>}
          </div>
